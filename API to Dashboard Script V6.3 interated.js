/** @OnlyCurrentDoc */
// Clean WhatsApp Messaging + Firebase Sync - V6.3 interated
...
const CONFIG = {
  ...
  // Cherry-pick #1: configurable batch controls
  BATCH_SIZE: 10,
...
class TwilioManager {
  ...
  static sendWhatsAppMessages() {
    ...
    const data = sheetManager.sheet.getDataRange().getValues();
    const batchSize = Math.max(1, Number(CONFIG.BATCH_SIZE) || 10);
    const batchDelayMs = Math.max(0, Number(CONFIG.BATCH_DELAY_SECONDS) || 0) * 1000;
    let processed = 0;
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if ((i % batchSize) === 0 && rowIndex < data.length && batchDelayMs > 0) {
        Logger.log("BatchPause", `Processed ${batchSize} rows. Pausing for ${CONFIG.BATCH_DELAY_SECONDS} seconds.`);
...
function doPost(e) {
  ...
  // Cherry-pick #2: backward-compatible outbound trigger alias
  if (params.triggerWhatsappOverList === "1") {
    try {
      ...
      Logger.log("WhatsAppTriggerError", "Outbound trigger failed", error.toString());
      return ContentService.createTextOutput("ERROR").setMimeType(ContentService.MimeType.TEXT);
    }
  }
...
function doGet(e) {
  ...
  return ContentService.createTextOutput("WhatsApp + Firebase Sync Service V6.3 interated is running").setMimeType(ContentService.MimeType.TEXT);
}
...
// Safe warmup: touches Firestore only (no WhatsApp send)
function manualFirestoreWarmup() {
  try {
    const sheetManager = new SheetManager();
    const data = sheetManager.sheet.getDataRange().getValues();

    if (!data || data.length < 2) {
      Logger.log("FirebaseWarmup", "No data rows found for warmup");
      return;
    }

    let targetRowIndex = null;
    for (let i = 1; i < data.length; i++) {
      if (!sheetManager.isRowEmpty(data[i])) {
    ...
    if (!targetRowIndex) {
      Logger.log("FirebaseWarmup", "No non-empty row found for warmup");
      return;
    }

    const warmupData = {
      firebaseWarmupAt: new Date(),
      firebaseWarmup: true,
    };

    FirebaseManager.syncRowToFirestore(sheetManager, targetRowIndex, warmupData);
    Logger.log("FirebaseWarmup", `Firestore warmup completed on row ${targetRowIndex}`);
  } catch (error) {
    Logger.log("FirebaseWarmupError", "Warmup failed", error.toString());
  }
}
