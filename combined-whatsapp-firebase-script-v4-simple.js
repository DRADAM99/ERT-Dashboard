/** @OnlyCurrentDoc */
// Combined Google Apps Script: WhatsApp Messaging + Firebase Sync V4 SIMPLE
//
// Goals in v4:
...
//
// SIMPLE: Handle malformed webhook parameters with basic string parsing
...
// ===== WEBHOOK ROUTING =====
function doPost(e) {
  logToSheetV4([new Date(), "Webhook", "POST received V4 SIMPLE"]);
...
function extractWebhookParamsV4(e) {
  ...
  try {
    ...
    // Handle malformed data from Twilio with simple string parsing
    if (e && e.postData && e.postData.contents) {
      var formData = e.postData.contents;
      logToSheetV4([new Date(), "Webhook Raw Data", formData]);
      
      // Simple extraction for the specific malformed format
      ...
      if (phoneMatch) {
        ...
        logToSheetV4([new Date(), "Phone Match", "Found phone: " + params.From]);
      } else {
        logToSheetV4([new Date(), "Phone Match", "No phone match found in: " + formData]);
      }
      ...
      if (bodyMatch) {
        ...
        logToSheetV4([new Date(), "Body Match", "Found body: " + params.Body]);
      } else {
        logToSheetV4([new Date(), "Body Match", "No body match found, trying keywords..."]);
      }
      ...
      if (!params.Body) {
        if (formData.indexOf('זקוקים') !== -1) {
          ...
          logToSheetV4([new Date(), "Keyword Match", "Found זקוקים -> " + params.Body]);
        } else if (formData.indexOf('בסדר') !== -1) {
          params.Body = 'הכל בסדר';
          logToSheetV4([new Date(), "Keyword Match", "Found בסדר -> " + params.Body]);
        } else if (formData.indexOf('בטוח') !== -1) {
          params.Body = 'לא בטוח';
          logToSheetV4([new Date(), "Keyword Match", "Found בטוח -> " + params.Body]);
        } else {
          logToSheetV4([new Date(), "Keyword Match", "No Hebrew keywords found in: " + formData]);
        }
      }
      ...
      if (params.From && params.Body) {
        ...
        logToSheetV4([new Date(), "Webhook Failed", "Missing From or Body. From: " + params.From + " Body: " + params.Body]);
      }
...
function handleTwilioInboundV4(params) {
  try {
    ...
    var timestampIndex = headers.indexOf('updated at');
    var statusIndex = headers.indexOf('סטטוס');
...
function nightlySyncRunV4() {
  try {
    ...
    syncResidentsToFirebaseV4(true);
...
function testRealWebhookV4() {
  // Test with the exact data from your logs
  var realData = '}"אנחנו זקוקים לסיוע=body":"From=whatsapp:+972543255956&Body"{';
  logToSheetV4([new Date(), 'Real Test', 'Testing real webhook data: ' + realData]);
  ...
  var params = extractWebhookParamsV4(e);
  logToSheetV4([new Date(), 'Real Test', 'Extracted params: ' + JSON.stringify(params)]);
  
  if (params.From && params.Body) {
    logToSheetV4([new Date(), 'Real Test', 'SUCCESS: From=' + params.From + ' Body=' + params.Body]);
  } else {
    logToSheetV4([new Date(), 'Real Test', 'FAILED: From=' + params.From + ' Body=' + params.Body]);
  }
}
...
function doGet(e) {
  ...
  return ContentService.createTextOutput("WhatsApp + Firebase Sync Service V4 SIMPLE is running").setMimeType(ContentService.MimeType.TEXT);
}
