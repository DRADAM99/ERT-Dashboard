...
export function phoneHref(phoneValue) {
  const onlyDigits = normalizePhoneDigits(phoneValue);
...
export function whatsAppHref(phoneValue) {
  const onlyDigits = normalizePhoneDigits(phoneValue);
  if (!onlyDigits) return null;
  if (onlyDigits.startsWith("972")) return `https://wa.me/${onlyDigits}`;
  if (onlyDigits.startsWith("0")) return `https://wa.me/972${onlyDigits.slice(1)}`;
