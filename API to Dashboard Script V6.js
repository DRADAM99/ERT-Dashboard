...
const CONFIG = {
  ...
  // Values that mean "no phone"
  INVALID_PHONE_PLACEHOLDERS: [
    'אין לי טלפון',
    'אין טלפון',
    'ללא טלפון',
    'לא ידוע',
...
class PhoneHelper {
  /**
   * Checks whether the raw cell value is a known placeholder for missing phone
  ...
  /**
   * Normalizes phone numbers to standard format (9725XXXXXXXX)
   * Returns empty string for invalid/missing placeholders
  ...
  static normalize(phoneNumber) {
    ...
    // Remove leading zero (e.g. 054... -> 54...)
    if (normalized.startsWith('0')) {
      normalized = normalized.substring(1);
    }

    // Add country code if missing (e.g. 54... -> 97254...)
  ...
  /**
   * True for Israeli mobile style numbers only (9725XXXXXXXX)
  ...
  /**
   * Determines if a raw phone value is safe/sendable
   * @param {string|number} phoneNumber
   * @returns {boolean}
   */
...
class PhoneLookupManager {
  ...
  static update() {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      // Safe rule: index only valid/sendable phones
...
class FirebaseManager {
  ...
  /**
   * Syncs a single row to Firestore database (creates or updates)
   * @param {SheetManager} sheetManager - Sheet manager instance
   * @param {number} rowIndex - 1-based row number to sync
   * @param {Object} additionalData - Additional data to merge into the row object
   */
  static async syncRowToFirestore(sheetManager, rowIndex, additionalData = {}) {
    try {
      const rowObj = sheetManager.buildRowObject(rowIndex);
      
      // Merge additional data (like message sending status)
...
class TwilioManager {
  ...
  /**
   * Formats phone number for Twilio API (9725XXXXXXXX format)
   * @param {string|number} phoneNumber - Raw phone number
  ...
  static formatPhoneNumber(phoneNumber) {
    ...
    if (!PhoneHelper.isValidIsraeliMobile(formatted)) {
      throw new Error(`Invalid or unsupported phone number: ${phoneNumber}`);
    }
  ...
  static sendWhatsAppMessages() {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      // Safe rule: skip placeholders/invalid phone text (e.g. "אין לי טלפון")
      if (!PhoneHelper.isSendablePhone(phoneNumber)) {
        Logger.log("WhatsAppSkip", `Skipping row ${i + 1}: invalid/non-sendable phone`, String(phoneNumber));
        continue;
      }
      ...
      try {
        ...
        const success = this.sendSingleMessage(credentials, formatted, sheetManager, i + 1);
        ...
        if (success) {
          ...
          if (!currentStatus || !userReplyStatuses.includes(String(currentStatus))) {
            ...
            Logger.log("StatusUpdate", `Updated row ${i + 1} status to MESSAGE_SENT`);
          }
          
          // Ensure Firebase record is created/updated for successful sends
          Logger.log("FirebaseCheck", `Verifying Firebase creation for row ${i + 1}`);
          
        } else {
          Logger.log("SendFailed", `Message send failed for row ${i + 1}, Firebase should still be updated via sendSingleMessage`);
        }
      } catch (error) {
        Logger.log("WhatsAppError", `Failed to send to row ${i + 1}`, error.toString());
        sheetManager.updateStatus(i + 1, `Error: ${error.toString()}`);
        
        // Also update Firebase with error status
        try {
          const errorData = {
          ...
          FirebaseManager.syncRowToFirestore(sheetManager, i + 1, errorData);
          Logger.log("FirebaseErrorSync", `Firebase updated with error for row ${i + 1}`);
        } catch (fbError) {
          Logger.log("FirebaseUpdateError", `Failed to update Firebase error for row ${i + 1}`, fbError.toString());
        }
      }
  ...
  /**
   * Sends a single WhatsApp message via Twilio API and updates Firebase
   * @param {Object} credentials - Twilio credentials
   * @param {string} phoneNumber - Formatted phone number (already normalized)
   * @param {SheetManager} sheetManager - Sheet manager instance
   * @param {number} rowIndex - Row index for Firebase sync
   * @returns {boolean} True if message sent successfully
   */
  static sendSingleMessage(credentials, phoneNumber, sheetManager = null, rowIndex = null) {
    ...
    // Update Firebase with sending status if sheetManager and rowIndex provided
    if (sheetManager && rowIndex) {
      try {
        const messageSendingData = {
          messageSentAt: new Date(),
          messageSentSuccess: success,
          twilioStatusCode: statusCode
        };
        
        if (!success) {
          messageSendingData.messageSentError = responseText;
        }
        
        FirebaseManager.syncRowToFirestore(sheetManager, rowIndex, messageSendingData);
        Logger.log("FirebaseUpdate", `Firebase created/updated for row ${rowIndex} with sending status: ${success}`);
        
      } catch (error) {
        Logger.log("FirebaseUpdateError", `Failed to update Firebase for row ${rowIndex}`, error.toString());
      }
    }
...
class WebhookHandler {
  ...
  static handleInboundMessage(params) {
    try {
      ...
      // UPDATE existing Firebase record with reply status (don't create new record)
      const replyData = {
